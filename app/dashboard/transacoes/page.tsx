  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingTx) {
      const { data, error } = await supabase
        ...
        .select()
        .single()
      if (!error && data) setTransactions(prev => prev.map(t => t.id === editingTx.id ? data : t))
    } else {
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        {[
          ...
          { label: 'Saldo',    value: totalIncome - totalExpenses, color: 'var(--ink)' },
        ].map(s => (
          <Card key={s.label}>
          ...
          <div>
            {filtered.map((tx, i) => (
              <div key={tx.id} style={{
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: '0.9rem', fontWeight: 500, margin: 0 }}>{tx.name}</p>
                ...
                <div style={{ display: 'flex', gap: '0.25rem' }}>
                  <button onClick={() => openEdit(tx)} title="Editar" style={{
                    ...
                    color: 'var(--ink-soft)', fontSize: '1rem', padding: '0.2rem 0.4rem',
                    borderRadius: '6px',
                  }}>✏️</button>
                  <button onClick={() => handleDelete(tx.id)} title="Excluir" style={{
                    background: 'none', border: 'none', cursor: 'pointer',
                    color: 'var(--ink-soft)', fontSize: '1rem', padding: '0.2rem 0.4rem',
                    borderRadius: '6px',
                  }}>🗑</button>
      ...
      <Modal open={showModal} onClose={closeModal} title={editingTx ? 'Editar lançamento' : 'Novo lançamento'}>
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
            ...
            <Button type="submit" style={{ flex: 1 }}>{editingTx ? 'Salvar alterações' : 'Salvar'}</Button>
          </div>
        </form>
      </Modal>
    </div>
